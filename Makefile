...
go-toolchain:	goios.ios64-new/README.md
	cd goios.ios64-new/src && GOROOT_BOOTSTRAP=${GOROOT_BOOTSTRAP} GOARM=7 CGO_ENABLED=1 GOARCH=arm CC_FOR_TARGET=`pwd`/../misc/ios/clangwrap.sh CXX_FOR_TARGET=`pwd`/../misc/ios/clangwrap.sh ./make.bash

ipfsios: 	go-toolchain
	cd ipfsios &&  xcodebuild -target ipfsios -arch armv7
